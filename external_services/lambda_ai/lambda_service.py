...
with open("lambda_config.yaml", 'r') as f:
...
def generate_ssh_key():
    ...
    auth = (API_KEY, "")
    open_file = config["PRIVATE_KEY"]

    headers = {config["HEADER_CONTENT_TYPE"]}
    data = {config["DATA"]}
...
def get_lambda_instance_vm_id():
    """Get the IP address of the Lambda instance VM."""
    intances = list_intances()
    for instance in intances["data"]:
        if instance["name"] == config["LAMDBA_INSTANCE_NAME"]:
            try:
                return instance["id"]
            except KeyError:
                print(config["LAMDBA_INSTANCE_NAME"] + "instance not found")
    return


def get_lambda_instance_vm_ip():
    """Get the IP address of the Lambda instance VM."""
    intances = list_intances()
    for instance in intances["data"]:
        if instance["name"] == config["LAMDBA_INSTANCE_NAME"]:
            try:
                print(f"Lambda Instance VM IP: {instance['ip']}")
            except KeyError:
                print("Lambda Instance VM IP not found")
    return
...
def terminate_instances():
    """Terminate the Lambda VM."""
    url = f"{BASE_URL}/instance-operations/terminate"
    auth = (API_KEY, "")
    headers = {"Content-Type": "application/json"}

    instance_id = get_lambda_instance_vm_id()
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("  get-ip         - Get Lambda Instance VM IP")
    ...
    commands = {
        ...
        "list-types": list_instance_by_types,
        "get-ip": get_lambda_instance_vm_ip,
